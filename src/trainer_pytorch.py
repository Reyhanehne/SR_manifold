...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (target, directory) in enumerate(self.data_loader):
            ...
            logits = self.model(target['input'], target['conv1'], target['conv2'], target['conv3'], target['conv4'],
                                target['c1'], target['c2'], target['c3'], target['c4'],
                                target['ver_num'])  # data: images, output: (params, R, logits)
    ...
    def _valid_epoch(self, epoch):
        ...
        #     self.writer.add_histogram(name, p.detach(), bins='auto')
        self.writer.close()
