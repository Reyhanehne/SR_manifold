...
def VGG16_1(input_layer, rgb_mean, num_out):
    one = np.zeros([100, cfg.num_out, 1, 1], dtype=np.float32)
    one[:, 0, :, :] = 1
    ...
    pool3_flat = tf.reshape(pool3, [-1, flattened_shape], name="flatten")
    fc1 = tf.reshape(fully_connected_f(pool3_flat, name='fc', n_out=cfg.num_out), [-1, cfg.num_out, 1, 1])  # *100
...
def polyval2d_loss(image, weight, degree):
    ...
    a = np.zeros([28, 28, 2]).astype(np.float32)
    for h in range(28):
        a[h, :, 0] = (-.5 + h / 28) * 2
        a[:, h, 1] = (-.5 + h / 28) * 2
    A = tf.constant(a)
